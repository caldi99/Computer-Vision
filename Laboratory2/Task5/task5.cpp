...
using namespace cv;
Mat computeHistogram(Mat& img, int bins, int rangeValuePixels);
...
int main(int argc, char** argv)
{
	...
	Mat histImage = computeHistogram(img, 256, 256);
	imshow("Example1", histImage);
	waitKey(0);

	Mat equalized;
	equalizeHist(img, equalized);
	imshow("Example1", equalized);
	waitKey(0);

	histImage = computeHistogram(equalized, 256, 256);
	imshow("Example1", histImage);
	waitKey(0);
...
Mat computeHistogram(Mat& img, int bins, int rangeValuePixels)
{
	float range[] = { 0, rangeValuePixels }; //the upper boundary is exclusive
